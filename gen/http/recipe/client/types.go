...
// NewListRxRecipeCollectionOK builds a "recipe" service "list" endpoint result
// from a HTTP "OK" response.
func NewListRxRecipeCollectionOK(body ListResponseBody) recipeviews.RxRecipeCollectionView {
	v := make([]*recipeviews.RxRecipeView, len(body))
	for i, val := range body {
		v[i] = &recipeviews.RxRecipeView{
			ID:          val.ID,
			Title:       val.Title,
		...
		if val.Ingredients != nil {
			v[i].Ingredients = marshalIngredientListResponseBodyToIngredientListView(val.Ingredients)
		}
	}
	return v
}
